...
<div class="form-inline" id="buttons">
    @if (isset($infrared) && isset($infrared->butttons))
        @foreach($infrared->buttons as $key => $button)

            <div class="form-inline" style="margin-bottom: 5px">
                {!! Form::text('buttons['.$key.'][id]', $button->id, ['style'=>'display:none;']) !!}
                &nbsp;Controle:&nbsp;
                {!! Form::select('buttons['.$key.'][device_id]', $devices, $button->code->device->id, ['class' => 'form-control', 'onchange' => 'getCodes(this)']) !!}
                &nbsp;Codigo:&nbsp;
                {!! Form::select('buttons['.$key.'][code_id]', \App\Code::where('device_id', $button->code->device->id)->pluck('name','id'), null, ['class' => 'form-control', 'onchange' => 'setDevice()']) !!}
                &nbsp;X:&nbsp;
...
<script>
    ...
    function addButton() {
        ...
        key++;
        var html = '&nbsp;Controle:&nbsp;'+
            '<select name="buttons['+key+'][device_id]" class="form-control" onchange="getCodes(this)">';
        $.map(devices, function(v, k){
            html += '<option value="'+k+'">'+v+'</option>';
        });
        html += '</select>'+
            '&nbsp;Codigo:&nbsp;'+
            '<select name="buttons['+key+'][code_id]" class="form-control"></select>'+
            '&nbsp;X:&nbsp;'+
